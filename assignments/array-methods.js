...
let allCaps = [];
allCaps.push(runners.map(x => x.first_name + " " + x.last_name));
console.log(allCaps);
...
// Problem 1
let longEmail = [];
longEmail.push(runners.filter(x => x.email.length > 25));
console.log(longEmail);

// Problem 2
let placement = [];
runners.forEach(function(currentValue) {
    placement.push(currentValue.id + " " + currentValue.last_name + " " + currentValue.first_name);
...
runners.map(function(currentValue) {
    abbreviation.push((x => x.first_name));
})
